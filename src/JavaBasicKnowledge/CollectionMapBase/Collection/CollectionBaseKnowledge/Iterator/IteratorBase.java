package JavaBasicKnowledge.CollectionMapBase.Collection.CollectionBaseKnowledge.Iterator;

import java.util.ArrayList;
import java.util.Collection;
import java.util.Iterator;

/**
 * 迭代器/遍历集合（重点：*******）
 *      迭代器之所以叫迭代器，是因为其内部包含的方法可以把集合中的元素一个一个的遍历出来
 */

public class IteratorBase {
    public static void main(String[] args) {
        //注意：以下讲解的遍历方式/迭代方式，是所有Collection通用的一种方式
        // 在Map集合中不能用，在所有的Collection以及子类中使用
        //创建集合对象
        Collection c = new ArrayList();//后面的集合无所谓，主要是看前面的Collection接口，怎么遍历，怎么迭代
        //添加元素

        c.add("abc");
        c.add("def");
        c.add(100);
        c.add(new Object());
        //对集合Collection进行遍历/迭代

        //第一步：获取集合对象的迭代器对象Iterator
                //在这里调用了c的迭代器方法，返回了一个迭代器对象

        Iterator it = c.iterator();

        //第二部：通过以上获取的迭代器对象开始迭代/遍历集合
        /**
         * 以下两个方法是迭代器对象Iterator中的方法
         *      boolean hasNext()如果下一位仍有元素可以迭代，则返回true
         *      Object next()返回迭代的下一个元素
         */
        /*
        boolean hasNext =  it.hasNext();
        if (hasNext){
            Object o = it.next();
            System.out.println(o);
        }

        hasNext =  it.hasNext();
        if (hasNext){
            Object o = it.next();
            System.out.println(o);
        }

        hasNext =  it.hasNext();
        if (hasNext){
            Object o = it.next();
            System.out.println(o);
        }

        hasNext =  it.hasNext();
        if (hasNext){
            Object o = it.next();
            System.out.println(o);
        }
         */
        //以上代码可以优化
        while (it.hasNext()){
            Object o = it.next();
            System.out.println(o);
        }
    }
}
