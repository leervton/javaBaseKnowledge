 ...
 *
 * 5.wait方法的作用：o.wait()让正在o对象上活动的线程t进入等待状态，并且释放掉t线程之前占有的o对象锁
 *
...
class Producer implements Runnable{
    ...
    public void run() {
            //一直生产(使用死循环来模拟一直生产)
            while (true){
                //给仓库对象list加锁，使代码块里面的代码带锁执行
                synchronized (list){
                     if (list.size()>0){//大于0，说明仓库中有了1个元素
...
class Customer implements Runnable{
    ...
    public void run() {
        ...
        while (true){
            synchronized (list){
                if (list.size()==0){//仓库空了，消费线程进入等待状态，释放掉list集合的锁
                    try {
                        list.wait();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
